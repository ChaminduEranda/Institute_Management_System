...
<?php
$query = mysqli_query($db_con, "SELECT * FROM `admin` WHERE `aId` ='$user';");
...
<div class="row">
  <div class="col-sm-6">
    ...
    <a class="btn btn-warning pull-right" href="admin.php?page=edit-userprofile&aId=<?php echo base64_encode($row['aId']); ?>">Edit Profile</a>
  </div>
  <div class="col-sm-6">
    ...
    if (isset($_POST['upphoto'])) {
      ...
      $upphoto = $user . '.' . $photo;
      if (mysqli_query($db_con, "UPDATE `admin` SET `photo` = '$upphoto' WHERE `admin`.`aId` = '$user';")) {
