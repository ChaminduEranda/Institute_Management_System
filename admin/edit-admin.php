...
if (isset($_POST['updateadmin'])) {
	...
	$email = $_POST['email'];
	$query = "UPDATE `admin` SET `aId`='$aId',`titleName`='$titleName',`name`='$name',`address`='$address',`dob`='$dob',`nic`='$nic',`phone`='$phone',`gender`='$gender',`photo`='$photo',`email`='$email' WHERE `aId`= '$aId'";
	if (mysqli_query($db_con, $query)) {
		$datainsert['insertsucess'] = '<p style="color: green;">admin Updated!</p>';
		if (!empty($_FILES['photo']['name'])) {
		...
		header('Location: admin.php?page=all-admins&edit=success');
	} else {
		header('Location: admin.php?page=all-admins&edit=error');
	}
}
?>
...
<nav aria-label="breadcrumb">
	<ol class="breadcrumb">
		...
		<li class="breadcrumb-item" aria-current="page"><a href="admin.php?page=all-admins">All admin </a></li>
	...
	<div class="col-sm-6">
		<form enctype="multipart/form-data" method="POST" action="">
			...
			<div class="form-group text-center">
				<input name="updateadmin" value="Update admin" type="submit" class="btn btn-danger">
			</div>
		</form>
	</div>
</div>