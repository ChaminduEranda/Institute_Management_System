...
if (isset($_POST['enroll'])) {
    ...
    $query = "INSERT INTO `tutor_enroll`(`tutor_id`, `class_id`) VALUES ('$tutor_id','$class_id');";
...
?>
<h1 class="text-primary"><i class="fas fa-book-open"></i> Enroll Tutor<small class="text-warning"> Enroll Tutor to Class!</small></h1>
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        <li class="breadcrumb-item" aria-current="page"><a href="admin.php">Dashboard </a></li>
        <li class="breadcrumb-item active" aria-current="page">Enroll Tutor</li>
    </ol>
</nav>
    ...
    <div class="col-sm-6">
        ...
        <form enctype="multipart/form-data" method="POST" action="">
            ...
            <div class="form-group">
                ...
                <select name="tutor_id" class="form-control" id="" value="<?= isset($tutor_id) ? $tutor_id : '' ?>" required>
...
<table class="table  table-striped table-hover table-bordered" id="data">
    ...
    <tbody>
        <?php
        $query = mysqli_query($db_con, 'SELECT * FROM `tutor_enroll` ORDER BY `tutor_enroll`.`tutor_id` DESC;');
        while ($result = mysqli_fetch_array($query)) { ?>
            <tr>
                <?php
                echo '
          <td>' . $result['tutor_id'] . '</td>
          <td>' . $result['class_id'] . '</td>
          <td>' . $result['reg_date'] . '</td>
              
          <td>
             &nbsp; <a class="btn btn-xs btn-danger" onclick="javascript:confirmationDelete($(this));return false;" href="admin.php?page=delete-enrolltu&tutor_id=' . base64_encode($result['tutor_id']) . '&class_id=' . base64_encode($result['class_id']) . '">
