...
$haha = mysqli_query($db_con, "SELECT * FROM `student` WHERE `sId`='$showuser';");
...
if (isset($_POST['save'])) {
    ...
    if (count($input_error) == 0) {
        $query = "SELECT * FROM `student` WHERE `password` = '$cp';";
        $result = mysqli_query($db_con, $query);
        if (mysqli_num_rows($result) == 1) {
            if (strlen($np) > 7) {
                $query = "UPDATE `student` SET `password` = '$np',`status` = '1'  WHERE `sId`='$sid';";
                $result = mysqli_query($db_con, $query);
                //header('Location: ../LoginForm.php');
                header('Location: student.php?page=student-dashboard&edit=success');
            } else {
...
<div class="col">
    <div class="col-md-12">
        <h1 style="color:red;"><i class="fas fa-key"></i> You must change your password to login ! </h1>
